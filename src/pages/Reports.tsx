...
export default function Reports() {
  ...
  const categoryChartData = CATEGORIES.map((cat, index) => ({
    name: `Cat. ${cat.id}`,
    fullName: cat.name,
    value: stockByCategory[cat.id],
    color: COLORS[index],
  }));

  const valueByCategory = CATEGORIES.map((cat, index) => {
    const categoryProducts = mockProducts.filter((p) => p.category === cat.id);
    const value = categoryProducts.reduce((sum, p) => sum + p.quantity * p.price, 0);
    return {
      name: `Cat. ${cat.id}`,
  ...
  return (
    <MainLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Analize și statistici despre stocul de produse pirotehnice
            </p>
          </div>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">{totalValue.toLocaleString()} RON</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">{lowStock.length} produse</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Detalii pe Categorii</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-muted/50">
                    ...
                    <th className="text-right p-3 font-semibold">Produse</th>
                    <th className="text-right p-3 font-semibold">Cantitate</th>
                    <th className="text-right p-3 font-semibold">Valoare</th>
                  </tr>
                </thead>
                <tbody>
                  {CATEGORIES.map((cat, index) => {
                    const catProducts = mockProducts.filter((p) => p.category === cat.id);
                    ...
                    return (
                      <tr key={cat.id} className="border-t hover:bg-muted/30">
                        ...
                        <td className="p-3 text-right">{catProducts.length}</td>
                        <td className="p-3 text-right font-semibold">
                          {stockByCategory[cat.id]}
                        </td>
                        <td className="p-3 text-right font-semibold">
                          {catValue.toLocaleString()} RON
                        </td>
                      </tr>
                    );
                  })}
